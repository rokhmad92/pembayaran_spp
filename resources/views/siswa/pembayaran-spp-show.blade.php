                        ...
                        <tbody>
                            @foreach ($pembayaran as $row)
                                <tr>
                                    ...
                                    <td>
                                        <a class="btn btn-success btn-sm"><i class=""></i>
                                            DIBAYAR</a>
                                    </td>
                                </tr>
                            @endforeach
                                    ...
                                    <td>
                                        ...
                                        @else
                                            <a class="btn btn-danger btn-sm"
                                                onclick="bayar('{{ $value['nama_bulan'] }}', '{{ $spp->tahun }}')"><i
                                                    class=""></i>
                                                {{ Universe::statusPembayaranBulan($value['nama_bulan'], $spp->tahun) }}
                                            </a>
                                        @endif
...
@push('js')
    ...
    <script type="text/javascript" src="https://app.sandbox.midtrans.com/snap/snap.js"
        data-client-key="{{ env('MIDTRANS_CLIENT_ID') }}"></script>
    <script src="{{ asset('jquery.min.js') }}"></script>
    <script>
        function bayar(namaBulan, tahun) {
            $.ajax({
                url: "{{ route('siswa.pembayaran-spp.snapToken') }}",
                data: {
                    _token: "{{ csrf_token() }}",
                    bulan: namaBulan,
                    tahun: tahun
