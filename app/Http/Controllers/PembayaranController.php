...
class PembayaranController extends Controller
{
    ...
    public function prosesBayar(Request $request, $nisn, notifWa $notifWa)
    {
        $request->validate([
        ...
        if (!$pembayaran) {
            DB::transaction(function () use ($request, $petugas, $notifWa) {
                foreach ($request->bulan_bayar as $bulan) {
                    ...
                    $notifWa->sendWa($siswa['no_telepon'], $siswa['nama_siswa'], $bulan);
                }
            });
            ...
            return back()->with(
                ...
                'Siswa dengan Nama: ' . $request->nama_siswa .
                    ', NISN: ' . $request->nisn .
                    ' sudah membayar SPP pada bulan: ' .
                    implode(', ', $pembayaran) .
                    ' di tahun: ' . $request->tahun_bayar .
